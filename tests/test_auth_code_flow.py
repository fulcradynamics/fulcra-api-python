import pytest
from fulcra_api.core import FulcraAPI, FULCRA_AUTH0_DOMAIN, FULCRA_AUTH0_CLIENT_ID, FULCRA_AUTH0_AUDIENCE, FULCRA_AUTH0_SCOPE
import urllib.parse
import datetime
...
def test_get_authorization_code_url_basic(client: FulcraAPI):
    ...
    assert url_str.startswith(f"https://{FULCRA_AUTH0_DOMAIN}/authorize?")
    
    parsed_url = urllib.parse.urlparse(url_str)
    query_params = urllib.parse.parse_qs(parsed_url.query)
    
    assert query_params["client_id"] == [FULCRA_AUTH0_CLIENT_ID]
    assert query_params["audience"] == [FULCRA_AUTH0_AUDIENCE]
    assert query_params["scope"] == [FULCRA_AUTH0_SCOPE]
